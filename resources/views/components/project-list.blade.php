        ...
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <div class="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                    <button @click="viewMode = 'list'" :class="viewMode === 'list' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'" class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all">{{__('List') }}</button>
                    <button @click="viewMode = 'board'" :class="viewMode === 'board' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'" class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all">{{__('Board') }}</button>
                </div>
   ...
   class="p-2 bg-slate-100 hover:bg-indigo-100 text-slate-400 hover:text-indigo-600 rounded-lg transition-colors">
    ...
    <a href="{{ route('projects.export', $project) }}" 
       class="text-[10px] font-black text-indigo-600 hover:text-indigo-800 uppercase tracking-widest flex items-center gap-1">
        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M6 2a2 2 0 00-2 2v12a2 2 0 002 2h8a2 2 0 002-2V7.414A2 2 0 0015.414 6L12 2.586A2 2 0 0010.586 2H6zm5 2l3.5 3.5H11V4z"/></svg>
        {{ __('Export PDF') }}
    </a>

    <form action="{{ route('projects.destroy', $project) }}" method="POST" onsubmit="return confirm('Delete?');">
        @csrf @method('DELETE')
        <button type="submit" class="text-[10px] font-bold text-red-400 hover:text-red-600 uppercase tracking-widest">
            {{ __('Delete Project') }}
        </button>
    </form>
</div>
@endcan
                        ...
                        <div class="flex items-center gap-6">
                            <div class="flex flex-col items-end gap-1">
                                <div class="flex items-center gap-3">
                                    <span class="text-[10px] font-black uppercase tracking-widest text-slate-400">{{ __('Progress') }}</span>
                            ...
                            <div class="hidden md:flex flex-col items-center px-4 border-l border-slate-200">
                                <span class="text-[10px] font-black uppercase tracking-widest text-slate-400">{{ __('Tasks') }}</span>
                ...
                <div class="divide-y divide-slate-50">
                    @foreach($project->tasks as $task)
                        <div class="group flex items-center justify-between p-4 hover:bg-slate-50 border-b border-slate-100 last:border-0 transition-colors">
                            <div class="flex items-center gap-4">
                                <span class="text-[10px] font-black uppercase px-2 py-1 rounded bg-slate-100 text-slate-500">{{ __(str_replace('_', ' ', strtoupper($task->status))) }}</span>
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <form action="{{ route('tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('Delete task?')" class="opacity-0 group-hover:opacity-100 transition-opacity">
                                    @csrf @method('DELETE')
                ...
                <footer class="p-4 bg-slate-50/50 border-t border-slate-100">
                    <form action="{{ route('tasks.store', $project->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-wrap md:flex-nowrap items-center gap-3">
                        @csrf
                        <input type="hidden" name="project_id" value="{{ $project->id }}">
                        <input type="text" name="title" placeholder="{{ __('Add a task') }}..." required class="flex-1 min-w-[200px] bg-white border-slate-200 rounded-xl text-sm px-4 py-2 border outline-none focus:ring-2 focus:ring-indigo-500/20">
                        ...
                        <input type="date" name="due_at" class="bg-white border-slate-200 rounded-xl text-[10px] font-bold px-3 py-2 border outline-none">
                        <button type="submit" class="bg-slate-900 text-white px-6 py-2 rounded-xl text-xs font-bold hover:bg-indigo-600 transition-all shadow-sm">{{ __('Add') }}</button>
                    </form>
                </footer>
            </section>
        </div>
        ...
        @foreach(['todo' => 'To Do', 'in_progress' => 'Active', 'review' => 'Review', 'done' => 'Done'] as $status => $label)
            <div class="min-w-[300px] md:w-1/4 flex-shrink-0 snap-start">
                <div class="flex items-center justify-between mb-4 px-2">
                    <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-400">{{ __($label) }}</h3>
                </div>
